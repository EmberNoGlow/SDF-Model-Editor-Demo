...
def handle_window_resize(window):
    ...
    return width, height, menu_bar_height, panel_width, rendering_width, rendering_height
...
def handle_keyboard_and_scene_input(window, io, scene_builder):
    ...
    handle = handler(window, io, scene_builder, glob_history, 
                     st.selected_item_id, st.selected_items)
...
def _is_shift_pressed(window):
    ...
    return (glfw.get_key(window, glfw.KEY_LEFT_SHIFT) == glfw.PRESS or 
...
def handle_camera_input(window, io, camera):
    ...
    st.cam_yaw, st.cam_pitch = camera.update(
        st.target_yaw, st.target_pitch, st.target_pan_y, st.target_pan_x,
...
def detect_camera_changes():
    ...
    camera_changed = (
        abs(st.cam_yaw - st.prev_cam_yaw) > epsilon or 
        abs(st.cam_pitch - st.prev_cam_pitch) > epsilon or
        abs(st.cam_radius - st.prev_cam_radius) > epsilon or
        any(abs(st.cam_orbit[i] - st.prev_cam_orbit[i]) > epsilon for i in range(3))
    )
...
def render_scene(window, scene_builder, camera, vao, vbo, display_vao, 
                 display_vbo, display_shader):
    ...
    use_accumulation = rendering_pass(
        st, st.shader, display_shader, vao, display_vao, st.uniform_locs,
    ...
    return use_accumulation, width, height, menu_bar_height, panel_width, rendering_width, rendering_height


def render_framebuffer_scaling(width, height, menu_bar_height, panel_width, 
                               rendering_width, rendering_height, display_shader):
    ...
    framebuffer_output, st.scaled_rendering_width, st.scaled_rendering_height, \
    fbo, render_texture, fbo_width, fbo_height = setup_framebuffer(
        st.scaled_rendering_width, st.scaled_rendering_height,
        st.fbo, st.render_texture, st.fbo_width, st.fbo_height
    )
    ...
    _render_scene_to_framebuffer(display_shader, render_texture, 
                                  width, height, rendering_width, rendering_height,
                                  panel_width, menu_bar_height)
    
    return fbo, render_texture, st.fbo_width, st.fbo_height, framebuffer_output


def _render_scene_to_framebuffer(display_shader, render_texture, width, height,
                                  rendering_width, rendering_height, panel_width, menu_bar_height):
...
def render_direct_to_screen(width, height, menu_bar_height, panel_width, 
                            rendering_width, rendering_height):
    ...
    if st.uniform_locs is not None:
        ...
        glUniform2f(st.uniform_locs['viewportOffset'], float(panel_width), float(menu_bar_height))
        ...
        glUniform3f(st.uniform_locs['CamOrbit'], st.cam_orbit[0], st.cam_orbit[1], st.cam_orbit[2])
...
def _render_sky_color_ui(scene_builder):
    ...
    top_color_changed, top_color_rgba = imgui.color_edit3(
        "SkyTopColor##color", st.sky_top_color[0], st.sky_top_color[1], st.sky_top_color[2]
    )
    ...
    bottom_color_changed, bottom_color_rgba = imgui.color_edit3(
        "SkyBottomColor##color", st.sky_bottom_color[0], st.sky_bottom_color[1], st.sky_bottom_color[2]
    )
...
def _render_export_volume_window(window, scene_builder):
    ...
    if imgui.button("Export", 135, 30):
        ...
        comp_bin = sdfexp.compute_sdf_3d(st.grid_size, st.vox_quality, code, 
                                         st.additional_scene_code, st.exp_use_color, window)
...
def _render_export_obj_window(window, scene_builder):
    ...
    if imgui.button("Export", 135, 30):
        ...
        success, message = save_sdfobj_dialog(sdfexp, dist_sdf, color_sdf, 
                                              st.export_z_up, elvl, st.exp_use_color)
...
def render_error_window():
    ...
    width = glfw.get_window_size(glfw.get_current_context())[0] if glfw.get_current_context() else 800
    height = glfw.get_window_size(glfw.get_current_context())[1] if glfw.get_current_context() else 600
...
def _render_scene_node_recursive(node_id, scene_builder, panel_width, depth):
    ...
    if children:
        for child_id in list(children):
            _render_scene_node_recursive(child_id, scene_builder, panel_width, depth + 1)
...
def _render_primitive_inspector(node, scene_builder, item_data, panel_width):
    ...
    _render_primitive_type_properties(primitive, node, scene_builder, panel_elem_width_vec3, panel_elem_width_float)
    ...
    changed, item_data.position = input_vec3(
        ...
        cn['STEP_VARIABLE_FLOAT'], panel_elem_width_vec3
    )
    if changed:
        success, new_uniforms = recompile_shader(scene_builder)
        if success:
    ...
    changed, item_data.rotation = input_vec3(
        ...
        cn['STEP_VARIABLE_ANGLE'], panel_elem_width_vec3
    )
    if changed:
        success, new_uniforms = recompile_shader(scene_builder)
        if success:
...
def _render_primitive_type_properties(primitive, node, scene_builder, panel_elem_width_vec3, panel_elem_width_float):
    ...
    if prim_type == "round_box":
        ...
        changed, primitive.kwargs['radius'] = input_float(
            "Radius", primitive.kwargs.get('radius', 0.1),
...
def _render_sprite_properties(primitive, node, scene_builder, panel_elem_width_vec3, panel_elem_width_float):
    ...
    changed_lod, spr.LOD = input_float(
        "LOD", spr.LOD, 0.1, panel_elem_width_float
    )
...
def _handle_primitive_selection(prim_type, label, size, scene_builder):
    ...
    if st.pending_change_node_id is None:
        ...
        scene_builder.change_node_to_primitive(
            st.pending_change_node_id, prim_type,
            position=None,
            size_or_radius=(size if size is not None else 0.5)
        )
        success, new_uniforms = recompile_shader(scene_builder)
...
def _handle_operation_selection(op_type, label, scene_builder):
    ...
    if st.pending_change_node_id is None:
        ...
        scene_builder.change_node_to_operation(
            st.pending_change_node_id, op_type,
            auto_primitive_type='box'
        )
        success, new_uniforms = recompile_shader(scene_builder)
        if success:
    ...
    if imgui.button("Reparent", 100, 30):
        if can_reparent:
            if scene_builder.reparent_node(
                st.reparent_node_id, st.reparent_target_parent, st.reparent_child_to_replace
            ):
                success, new_uniforms = recompile_shader(scene_builder)
                if success:
                    st.uniform_locs = new_uniforms
    ...
    for child_id in node.children:
        ...
        if child_node and child_node.node_type == 'operation':
            result |= _render_reparent_node_list(
                child_id, exclude_node_id, exclude_descendants, scene_builder,
                indent + "  "
            )
...
def _render_reparent_operand_selection(parent_id, scene_builder):
    ...
    required_operands = scene_builder._get_operand_count(parent_node.item_data.operation_type)
...
def render_fps_overlay(width, panel_width):
    ...
    imgui.begin(
        ...
        imgui.WINDOW_ALWAYS_AUTO_RESIZE | imgui.WINDOW_NO_SCROLLBAR
    )
...
def render_about_window(width, height):
    ...
    is_open, st.show_about_window = imgui.begin(
        "About", True, imgui.WINDOW_NO_COLLAPSE
    )
...
def main():
    ...
    while not glfw.window_should_close(window):
        ...
        use_accumulation, width, height, menu_bar_height, panel_width, rendering_width, rendering_height = \
            render_scene(window, scene_builder, camera, vao, vbo, st.display_vao, st.display_vbo, st.display_shader)
        
        # Framebuffer rendering
        if not (st.shader is not None and st.shader_choice == 1 and use_accumulation == 1):
            render_framebuffer_scaling(width, height, menu_bar_height, panel_width,
                                       rendering_width, rendering_height, st.display_shader)
            render_direct_to_screen(width, height, menu_bar_height, panel_width,
                                    rendering_width, rendering_height)
    ...
    # Cleanup
    cleanup_resources(st.fbo, st.render_texture, st.display_shader, st.display_vao, st.display_vbo, vao, vbo)
    impl.shutdown()
    glfw.terminate()
