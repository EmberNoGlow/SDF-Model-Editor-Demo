...
def change_node_to_operation(builder, node_id: str, operation_type: str, auto_primitive_type: str = 'box') -> bool:
    ...
    for i, oid in enumerate(operand_ids):
        if i == 0:
            ...
            prim = SDFPrimitive(builder.selected_item_id, auto_primitive_type, [1.0 * i, 0.0, 0.0], 0.5, ui_name=f"{auto_primitive_type.title()} {i+1}")
...
def change_node_to_primitive(builder, node_id: str, primitive_type: str = 'box', position=None, size_or_radius=None, rotation=None, scale=None, color=None, **kwargs) -> bool:
    ...
    prim = SDFPrimitive(builder.selected_item_id, primitive_type, pos, s_or_r, rotation or [0.0,0.0,0.0], scale or [1.0,1.0,1.0], ui_name=primitive_type, color=color or [0.8,0.6,0.4], **(kwargs or {}))
