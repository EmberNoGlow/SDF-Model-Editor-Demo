...
def rebuild_imgui_fonts(
    renderer: GlfwRenderer,
    base_font_path="path/to/your/font.ttf",
    base_font_size=16.0,
    UI_SCALE=1.0,
):
    """
...
def HSpinner(
    ...
    if imgui.is_item_active() and imgui.is_mouse_down(0):  # Left mouse button
        ...
        if (
            not state["has_repeated"]
            and (current_time - state["first_press_time"]) >= repeat_delay
        ):
            value -= value_step
            btn_changed = True
        ...
        # Check if enough time has passed for subsequent repeats
            ...
            state["has_repeated"]
            and (current_time - state["last_repeat_time"]) >= repeat_rate
        ):
            value -= value_step
            btn_changed = True
            state["last_repeat_time"] = current_time
